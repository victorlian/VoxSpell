...
public class AccuracyChart extends JComponent{
	...
	protected void paintComponent (Graphics g){	
		...
		//To calculate where to put the string, think as: leftLength+string+rightLength=Width
		//String width use font metrics.
		int masteredTextWidth = g.getFontMetrics().stringWidth("mastered");
		int faultedTextWidth = g.getFontMetrics().stringWidth("faulted");
		int failedTextWidth = g.getFontMetrics().stringWidth("faulted");
		...
		if (masteredWidth!=0){		
			if (masteredWidth>masteredPercentWidth){
				g.drawString("mastered", xCo+calculateXCoText(masteredTextWidth, masteredWidth), yCo+HEIGHT*1/3);
				g.drawString(masteredPercent, xCo+calculateXCoText(masteredPercentWidth, masteredWidth), yCo+HEIGHT*2/3);
			}

		}
		if (faultedWidth!=0){	
			if (faultedWidth>faultedPercentWidth){
				g.drawString("faulted", xCo+masteredWidth+calculateXCoText(faultedTextWidth, faultedWidth), yCo+HEIGHT*1/3);
				g.drawString(faultedPercent, xCo+masteredWidth+calculateXCoText(faultedPercentWidth, faultedWidth), yCo+HEIGHT*2/3);
			}
		}
		if (failedWidth!=0){			
			if (failedWidth>failedPercentWidth){
				g.drawString("failed", xCo+masteredWidth+faultedWidth+calculateXCoText(failedTextWidth, failedWidth), yCo+HEIGHT*1/3);
				g.drawString(failedPercent, xCo+masteredWidth+faultedWidth+calculateXCoText(failedPercentWidth, failedWidth), yCo+HEIGHT*2/3);
			}
		}
